    ...
    <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
      ...
      <section className="space-y-6">
        {!selected ? <div className="rounded-2xl border border-dashed p-12 text-center text-slate-500">Chọn hoặc tạo đội để quản lý.</div> : <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Users className="h-5 w-5 text-emerald-600" /><h2 className="font-black text-slate-950">Thành viên ({selected.members?.length ?? 0})</h2></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{(selected.members ?? []).map((member) => <div key={member.userId} className="flex items-center gap-3 rounded-xl border border-slate-100 p-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-xs font-bold">{member.userId.slice(0, 2).toUpperCase()}</span><span className="min-w-0 flex-1"><b className="block truncate text-xs">{member.userId}</b><select value={member.role} onChange={(event) => void changeRole(member.userId, event.target.value as 'CAPTAIN' | 'MANAGER' | 'PLAYER')} className="mt-1 rounded border border-slate-200 px-1.5 py-1 text-[11px] font-semibold"><option value="CAPTAIN">Đội trưởng</option><option value="MANAGER">Quản lý</option><option value="PLAYER">Cầu thủ</option></select></span>{member.role !== 'CAPTAIN' && <button type="button" onClick={() => remove(member.userId)} className="text-rose-500" aria-label="Xóa thành viên"><Trash2 className="h-4 w-4" /></button>}</div>)}</div></div>
