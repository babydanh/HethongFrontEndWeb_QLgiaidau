...
export default function CommunityDetailPage() {
  ...
  const handleCancelJoinRequest = async () => {
    ...
    try {
      setIsJoinLoading(true);
      await communitiesApi.removeMember(id, currentUserId);
      toast.success(translate('cancelJoinRequestSuccess'));
      setMembership(null);
      setIsCancelRequestConfirmOpen(false);
      fetchCommunity();
    } catch (error) {
      console.error('Failed to cancel join request', error);
      toast.error(getErrorMessage(error, translate('cancelJoinRequestFailed')));
    } finally {
      setIsJoinLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      ...
      <ConfirmModal
        ...
        onOpenChange={setIsCancelRequestConfirmOpen}
        title={translate("cancelJoinRequestTitle")}
        description={translate("cancelJoinRequestDescription")}
        confirmLabel={translate("cancelJoinRequestConfirm")}
        variant="danger"
        isLoading={isJoinLoading}
