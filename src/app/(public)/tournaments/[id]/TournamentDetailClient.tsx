      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8 mt-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
          ...
          <div className="lg:col-span-3 space-y-6 min-w-0 max-w-full overflow-hidden">
            ...
            {/* Unpaid payment reminder banner for registered user */}
            {myRegistration?.registered && !myRegistration.participant?.isPaid && Number(activeTournament.entryFee ?? 0) > 0 && (
              <div className="mb-4 bg-gradient-to-r from-amber-500 to-amber-600 rounded-xl p-4 text-white shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border border-amber-400 animate-in fade-in duration-300">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-white/20 flex items-center justify-center shrink-0">
                    <CreditCard className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <p className="font-bold text-sm text-white">Bạn đã đăng ký nhưng chưa hoàn tất nộp lệ phí thi đấu</p>
                    <p className="text-xs text-amber-100 font-medium mt-0.5">Vui lòng nộp lệ phí sớm để đảm bảo giữ chỗ thi đấu chính thức trong giải đấu.</p>
                  </div>
                </div>
                <Link href={registerHref} className="shrink-0 w-full sm:w-auto">
                  <Button className="w-full sm:w-auto bg-white hover:bg-slate-100 text-amber-950 font-extrabold text-xs px-4 py-2 rounded-lg shadow-sm">
                    💳 Thanh toán ngay
                  </Button>
                </Link>
              </div>
                    ...
                    {divisionsList.map((division) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <span className={`transition-transform duration-300 ease-out ${isActive ? 'rotate-90' : ''}`}>
                              <ChevronRight className={`h-5 w-5 ${isActive ? 'text-blue-600' : 'text-slate-400'}`} aria-hidden="true" />
                            </span>
