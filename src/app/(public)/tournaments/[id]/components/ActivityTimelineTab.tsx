...
/**
 * Historical momentum calculation:
 * For each match in chronological order (oldest to newest),
 * calculate the win/loss streak each team has accumulated up to that point.
 */
...
function computeChronologicalStreaks(matches: Match[]): Map<string, { p1Streak: TeamStreakRecord | null; p2Streak: TeamStreakRecord | null }> {
  const streakMap = new Map<string, { p1Streak: TeamStreakRecord | null; p2Streak: TeamStreakRecord | null }>();

  // Filter completed matches and sort chronologically (oldest first)
  ...
  // Track running streak per participant ID
  ...
  for (const m of sorted) {
    ...
    // Snapshot current streaks before this match results update
    ...
    // If finished, update running streaks for subsequent matches
...
/**
 * Shape-matched skeleton loader following taste-skill rules
 */
function ActivityTimelineSkeleton() {
  return (
    <div className="space-y-6 pt-2" aria-label="Loading activity feed">
...
/**
 * Minimalist Streak Micro-Pill
 * Strictly neutral & understated palette (anti-slop, no rainbow colors)
 */
...
export default function ActivityTimelineTab({ tournament, tournamentId, divisionId }: Props) {
  const searchInputId = useId();
  const translate = useTranslations('TournamentDetail');
  const matchTranslate = useTranslations('Match');
  const openUserProfile = useUserProfileModalStore((state) => state.openUserProfile);
  ...
  const fetchTimelineMatches = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      // Fallback silent failure to prevent breakages
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    ...
    const handleMatchUpdate = (updatedMatch: Match) => {
      ...
      if (updatedMatch.tournamentId && updatedMatch.tournamentId !== tournamentId) return;
      if (divisionId && updatedMatch.divisionId && updatedMatch.divisionId !== divisionId) return;
  ...
  // Compute streaks based on all matches chronologically
  const streaksMap = useMemo(() => {
    return computeChronologicalStreaks(matches);
  }, [matches]);

  // Filter and sort for timeline view (most recent activity first)
    ...
    return matches
      .filter((m) => {
        ...
        if (query) {
          ...
          if (!t1.includes(query) && !t2.includes(query) && !m1 && !m2) {
            return false;
          }
        }
        return true;
      ...
      .sort((a, b) => {
        // Ongoing first, then completed by completedAt / updatedAt desc
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-slate-200">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-slate-900 tracking-tight">
              {translate('tabs.activity')}
            </h3>
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-700">
              {matches.length} {translate('matchesLabel')}
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Dòng sự kiện trận đấu thực tế và chuỗi phong độ (Streak) của các đội
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => void fetchTimelineMatches(true)}
      ...
      {isLoading && matches.length === 0 ? (
        ...
        <div className="text-center py-16 border border-dashed border-slate-200 rounded-xl bg-white text-slate-500">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Các trận đấu khi diễn ra hoặc kết thúc sẽ xuất hiện theo dòng thời gian tại đây.
          </p>
        </div>
      ) : (
