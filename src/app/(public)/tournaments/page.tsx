  ...
  return (
    <div className="flex-grow w-full max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
          ...
          <div className="relative w-full sm:w-40 shrink-0">
            <select
              ...
              className="w-full pl-3 pr-8 py-2 bg-slate-100 hover:bg-slate-100/80 focus:bg-white border border-transparent focus:border-blue-500 rounded-xl text-xs font-bold text-slate-800 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all h-10 cursor-pointer shadow-2xs"
            >
              <option value="">{translate("statusLabel")}: {translate("all")}</option>
          ...
          {/* Nút Bộ lọc thêm (Icon Button cực gọn) */}
          <button
            type="button"
            onClick={() => setShowAdvancedFilters(!showAdvancedFilters)}
            title={moreFiltersLabel}
            aria-label={moreFiltersLabel}
            className={`w-full sm:w-10 h-10 rounded-xl flex items-center justify-center transition-all cursor-pointer shrink-0 border ${
        ...
        {showAdvancedFilters && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-slate-50/90 rounded-2xl border border-slate-200/80 shadow-xs animate-in fade-in slide-in-from-top-2 duration-200">
        ...
        {/* Quick status filters — Inline chips gọn nhẹ sát thanh tìm kiếm */}
        <div className="flex flex-wrap items-center gap-1.5 pt-0.5 text-xs font-semibold" aria-label={translate("quickFilter")}>
          <button
            type="button"
            aria-pressed={selectedStatus === 'IN_PROGRESS'}
            onClick={() => setSelectedStatus(selectedStatus === 'IN_PROGRESS' ? '' : 'IN_PROGRESS')}
            className={`rounded-full px-3 py-1 text-[11px] font-bold transition-all cursor-pointer shadow-2xs border ${
          ...
          <button
            ...
            className={`rounded-full px-3 py-1 text-[11px] font-bold transition-all cursor-pointer shadow-2xs border ${
              selectedStatus === 'REGISTRATION_OPEN'
                ...
                : 'bg-emerald-50/80 text-emerald-700 border-emerald-200/80 hover:bg-emerald-100'
            }`}
          >
            ...
            return (
              ...
              >
                ...
                <div className="p-5 flex gap-5 flex-grow">
                  ...
                  <div className="flex items-center justify-center shrink-0 border-r border-slate-100 pr-5 min-w-[90px]">
                    <div className="flex items-center gap-1.5 font-bold text-slate-900 leading-none">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-[11px] font-bold text-slate-500 mt-0.5">T{startMonth}</span>
                      </div>

                      {/* Dấu gạch ngang ở giữa */}
                      <span className="text-slate-400 font-bold text-base mb-3">-</span>

                      {/* Ngày kết thúc / Tháng mini */}
                      <div className="flex flex-col items-center">
                        <span className="text-2xl font-black">{endDay}</span>
                        <span className="text-[11px] font-bold text-slate-500 mt-0.5">T{endMonth}</span>
                      </div>
                    </div>
                  </div>
