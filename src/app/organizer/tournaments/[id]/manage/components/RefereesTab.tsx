...
export function RefereesTab({
  ...
  const refereeTranslate = useTranslations('OrganizerReferees');
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-6 md:p-8 shadow-sm space-y-6 animate-in fade-in duration-200">
      <div className="border-b pb-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">{refereeTranslate('title')}</h2>
          <p className="text-xs text-slate-455 mt-1 font-semibold">{refereeTranslate('description')}</p>
        </div>
      </div>
      ...
      >
        <div className="flex-1 w-full">
          <label className="block text-xs font-bold text-slate-700 mb-1.5">{refereeTranslate('refereeEmail')}</label>
          <Input
            type="email"
        ...
        >
          {isAddingReferee ? refereeTranslate('sendingInvite') : refereeTranslate('sendInvite')}
        </Button>
      </form>
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-slate-900 flex items-center gap-1.5">
          <Shield className="w-5 h-5 text-blue-600" /> {refereeTranslate('listTitle')}
        </h3>
          ...
          {referees.map((ref) => (
            <div key={ref.id} className="flex items-center gap-3 bg-white p-4 rounded-lg border border-slate-200 shadow-sm relative group overflow-hidden">
              <div className="w-10 h-10 rounded-full bg-blue-50 border border-slate-200 flex items-center justify-center font-bold text-xs text-blue-650 shrink-0 uppercase overflow-hidden">
                {ref.avatarUrl ? (
                  <img src={ref.avatarUrl} alt={refereeTranslate('avatarAlt')} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="min-w-0">
                ...
                <div className="text-[10px] text-slate-455 font-semibold mt-0.5">{refereeTranslate('status')} 
                  <span className="ml-1 text-emerald-650 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 font-bold">{refereeTranslate('active')}</span>
                </div>
              </div>
            </div>
          ))}
