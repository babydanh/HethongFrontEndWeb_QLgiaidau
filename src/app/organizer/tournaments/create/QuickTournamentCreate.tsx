...
const quickSchema = z.object({
  ...
  entryFee: z.number().int().min(0).max(100000000),
  ...
  maxReserve: z.number().int().min(0).max(30),
...
export default function QuickTournamentCreate() {
  ...
  const { register, handleSubmit, setValue, control, formState: { errors } } = useForm<QuickValues>({
    ...
    defaultValues: {
      ...
      maxTeams: 16, startDate: today(), startTime: '18:00', registrationEndDate: '', entryFee: 0,
  ...
  const entryFee = useWatch({ control, name: 'entryFee' });
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-10 md:px-8">
      <section className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Phí tham gia (đồng)<input type="number" min={0} {...register('entryFee', { valueAsNumber: true })} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal" />{entryFee > 0 && <span className="mt-1 block text-xs text-amber-700">Giải có phí nên kiểm tra thanh toán trong Advanced.</span>}</label><label className="text-sm font-semibold text-slate-700">Tên sân<input {...register('venueName')} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal" placeholder="Không bắt buộc" /></label></div>
