...
export default function FootballTeamEloCard({ team, ranking, position }: Props) {
  ...
  const tierLabel = matchesPlayed > 0 ? rank?.tierName || 'Đang xếp hạng' : 'Chưa xếp hạng';

  return (
    <section className="rounded-xl border border-emerald-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">ELO đội bóng</span>
        <Link href="/football-teams" className="text-[11px] font-bold text-emerald-700 hover:text-emerald-800">Quản lý đội</Link>
      </div>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[11px] text-slate-500">{team.membership?.role === 'CAPTAIN' ? 'Đội trưởng' : team.membership?.role === 'MANAGER' ? 'Quản lý đội' : 'Thành viên'}</p>
        </div>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2 border-t border-slate-100 pt-3">
        ...
        <div><p className="text-[10px] text-slate-400">Hạng</p><p className="text-sm font-black tabular-nums text-slate-900">{position ? `#${position}` : '—'}</p></div>
      ...
      <p className="mt-3 flex items-center gap-1.5 text-[11px] text-slate-500">
        ...
        {matchesPlayed > 0 ? `${matchesPlayed} trận · ${rank?.matchesWon ?? 0} thắng` : 'Thi đấu trận hợp lệ để có thứ hạng'}
      </p>
    </section>
  );
}
