...
export function AthleteProfileCard({
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden text-center p-3.5 animate-pulse min-h-[248px] flex flex-col justify-between">
        <div>
          {/* Avatar skeleton */}
          <div className="w-13 h-13 rounded-full bg-slate-200 mx-auto mb-2" />
          {/* User name skeleton */}
          <div className="h-4 bg-slate-200 rounded w-28 mx-auto mb-1.5" />
          {/* Elo & Tier skeleton */}
          <div className="h-3 bg-slate-200 rounded w-36 mx-auto mb-2.5" />
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden text-center p-3.5 min-h-[248px] flex flex-col items-center justify-center">
        <div className="w-12 h-12 rounded-full border-2 border-slate-200 bg-slate-50 mx-auto mb-2 flex items-center justify-center">
        ...
        <p className="text-xs text-slate-500 mt-1 leading-relaxed max-w-[220px]">
          {translate('loginToViewProfile')}
        </p>
        <Link
          href="/login?redirect=/"
          className="inline-flex items-center justify-center mt-3 px-3.5 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-bold hover:bg-blue-700 transition-colors shadow-xs"
        >
          {translate('signInNow')}
        </Link>
      </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden text-center p-3.5 transition-all min-h-[248px] flex flex-col justify-between">
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs p-3.5 flex flex-col justify-between min-h-[248px]">
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Link
            href={isAuthenticated ? '/communities' : '/login?redirect=/'}
        ...
        {displayClubs.length === 0 ? (
          <div className="p-3 rounded-lg border border-dashed border-slate-200 bg-slate-50/50 text-center flex flex-col items-center justify-center gap-1.5 h-[170px]">
            ...
            >
              {isAuthenticated ? 'Khám phá CLB ngay' : translate('signInNow')} <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
        ) : (
